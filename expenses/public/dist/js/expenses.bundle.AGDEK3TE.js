(() => {
  // ../expenses/expenses/public/js/expenses.bundle.js
  ...
  var Expenses = class {
    ...
    request(method, args, callback, error, _freeze) {
      ...
      let opts = {
        ...
        callback: $.proxy(function(ret) {
          ...
          if (!error)
            ...
            error.call(this, { message: __(message, args) });
        }, this),
        error: $.proxy(function(ret, txt) {
          let message = "";
          if (this.$isStr(ret))
            message = ret;
          else if (this.$isStr(txt))
            message = txt;
          else
            message = "The request sent have failed.";
          if (!error)
            this.error(message, args);
          else
            error.call(this, { message: __(message, args) });
      ...
      try {
        ...
        if (error)
          ...
          this._error("Error: " + e.message, e.stack);
        if (this.has_error)
          throw e;
      } finally {
    ...
    _event_adder(event, fn, _once, _realtime) {
      if (!this.$isStr(event) || !event.length || !this.$isFunc(fn))
        return this;
      event.split(" ").forEach($.proxy(function(e) {
    ...
    _events_clear(event, all) {
      if (!all && this._events.list[event].length)
        return;
    ...
    enable_form(frm, workflow) {
      try {
        ...
        if (!workflow || frm.doc.__islocal || workflow && !frm.states.get_state())
          frm.enable_save();
        else
    ...
    get_field(frm, key, cdn, fkey, form) {
      ...
      if (field && field.grid && cdn && fkey) {
        ...
        if (form && row.grid_form && row.grid_form.fields_dict)
    ...
    focus(frm, key, cdn, fkey, form) {
      ...
      if (row && row.row && form) {
        ...
        if (field.length)
          ...
          row.row.find('input[type="Text"],textarea,select').filter(":visible:first").focus();
        }
      } else {
    ...
    valid_field(frm, key, cdn, fkey, form) {
      ...
      if (field.set_description) {
        ...
        if (field.toggle_description && (!field.df || !cstr(field.df.description).length))
    ...
    $type(v) {
      let t = v == null ? v === void 0 ? "Undefined" : "Null" : Object.prototype.toString.call(v).slice(8, -1);
    ...
    $isFunc(v) {
      return typeof v === "function" || /(Function|^Proxy)$/.test(this.$type(v));
    }
    $isArr(v) {
      return $.isArray(v);
    }
    ...
    _alert(title, msg, args, def_title, indicator, fatal) {
      ...
      if (!msg || !this.$isStr(msg))
        msg = __("Invalid message");
      else if (args)
        msg = __(msg, args);
      else
        msg = __(msg);
      if (!title || !this.$isStr(title))
        title = def_title;
      let data = {
    ...
    add() {
      ...
      if (i >= 0) {
        for (let k in this._c) {
          if (k !== 0)
            this._c[k][i] = arguments[k];
        }
      } else {
        for (let i2 = 0, l = arguments.length; i2 < l; i2++) {
          if (arguments[i2] != null)
            this._c[i2].push(arguments[i2]);
        }
      }
      return this;
    }
  ...
  frappe.exp = function(opts) {
    if (!frappe.exp._init)
      frappe.exp._init = new Expenses(opts);
    else
      frappe.exp._init.set_options(opts);
    return frappe.exp._init;
  };
})();
//# sourceMappingURL=expenses.bundle.AGDEK3TE.js.map
